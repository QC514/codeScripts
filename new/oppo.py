...
1. VMPF 平台：（WMPF 微信小程序协议服务平台，覆盖取 code）
   ...
   - 请求体：{"Appid": "wxe705c556754a1de2", "Wxid": "<微信 openid>"}
...
2. 账号变量：
   qingyun_openid                                    调用的 VMPF 账号 Wxid 列表
   - 多账号支持使用 & 或换行分隔
...
def get_code():
    ...
    payload = {"Appid": APPID, "Wxid": WXID}
    headers = {"Content-Type": "application/json", "X-Api-Key": VMPF_API_KEY}
    status, _, rbody = _http("POST", url, headers=headers, body=payload, timeout=30)
    try:
        j = json.loads(rbody.decode("utf-8"))
    except Exception:
        raise RuntimeError(f"VMPF 响应解析失败: {rbody[:200]!r}")
    ...
    if not code:
        raise RuntimeError(f"VMPF JSLogin 未返回 code: {json.dumps(j, ensure_ascii=False)[:200]}")
    return code
...
def main():
    if not VMPF_URL or not VMPF_API_KEY:
        log("❌ 未配置 VMPF_URL / VMPF_API_KEY 环境变量 (VMPF 平台地址与 API 密钥)")
        return 1
    if not WXID:
        log("❌ 未配置 qingyun_openid 环境变量 (VMPF 账号 Wxid 列表)")
        return 1
