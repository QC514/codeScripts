 ...
 *
 * ------------------------------------------------------------
 * VMPF 平台配置说明（环境变量，缺失即报错退出）：
 *   VMPF_URL         平台接口地址（不含路径），如 http://127.0.0.1:5679
 *   VMPF_API_KEY     API 密钥（wmpf_xxx），通过请求头 X-Api-Key 鉴权
 *   qingyun_openid   openid 列表，多账号用 & 或换行分隔（每行=一个账号）
 * 取 code（业务登录前，标准 JSLogin 协议）：
 *   POST {VMPF_URL}/api/wxapp/JSLogin
 *   Headers: X-Api-Key: <VMPF_API_KEY>
 *   Body: {"Appid":"wx340f915763f3ed2b","Wxid":"<openid>"}
 *   成功判定: Code===0，code 在 Data.code
 * ------------------------------------------------------------
 *
 * ============================================================
 * 抓取字段总表：
 ...
 *   ③ 无兜底/缓存→VMPF POST /api/wxapp/JSLogin 取 code→业务登录→写缓存
 *   ④ 都失败→判失败跳下一账号（同轮内对某账号 VMPF 仅取一次 code）
 *
 ...
 *     （openid 即 qingyun_openid 里的 openid；多账号换行分隔）
 *   命中后脚本不再走 VMPF 取号，也不写本地缓存；TOKEN 失效时自动回退到 ②/③ 重登。
 * ============================================================
 */
const fs = require('fs');
...
// ============ 配置（VMPF 平台规范，文档第 2 节；业务专属变量用 PAID_ 前缀） ============
...
// 账号来源：qingyun_openid（多账号用 & 或换行分隔，每行一个 openid）
...
const NOTIFY_ON_FAIL_ONLY = (process.env.NOTIFY_ON_FAIL_ONLY || process.env.PAID_NOTIFY_ON_FAIL_ONLY || 'true').trim().toLowerCase() !== 'false'; // 全局无前缀；兼容旧 PAID_ 前缀

// ============ 业务常量 ============
const APP = { name: '平安i动', appid: 'wx340f915763f3ed2b' };      // 取 code 时传给 JSLogin 的 Appid（沿用原脚本）
const APPID = APP.appid;                                            // 兼容旧引用 / 导出
...
// ============== VMPF 取 code（标准 JSLogin 协议；脚本无 axios 依赖，用原生 fetch 实现） ==============
...
async function getCode(openid) {
  ...
  if (!ref) return null;
  try {
    const resp = await fetch(VMPF_URL + '/api/wxapp/JSLogin', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-Api-Key': VMPF_API_KEY },
      body: JSON.stringify({ Appid: APP.appid, Wxid: ref }),
      signal: AbortSignal.timeout(20000),   // 对应模板 axios timeout: 20000
    });
...
async function runOne(openid, cache, tkmap){
  ...
  try {
    ...
    // ③ VMPF JSLogin 取 code 登录（仅当无兜底TOKEN、无缓存、或登录首次调用失败）
    if (!auth){
      auth = await doLogin(openid); src = 'VMPF 登录成功';
      cache[openid] = auth; saveCache(cache);
    }
    line.push('🔑 ' + (tkmap[openid] ? 'TOKENS 兜底有效' : (src.includes('缓存') ? '缓存有效→复用' : '缓存已失效')));
    if (src.includes('VMPF')) line.push('✅ VMPF 取 code 登录成功（已更新缓存）');
    ...
    try {
      ...
      if (sj.CODE === '10000' && /登录失效|请重新登录/.test(sj.MSG || '') && !relogged){
        ...
        line.push('🔄 缓存失效，已重新登录（VMPF 取 code）');
...
// ============================== Bark ==============================
// Bark 推送（BARK_PUSH 四态解析 + NOTIFY_ON_FAIL_ONLY；失败标题带 ⚠️ 标识）
...
// ============================== 主流程 ==============================
async function main(){
  // 账号来源：qingyun_openid（每行/每个 & 一个 openid，SERVERS 上方已校验非空）
  const accounts = SERVERS.map(openid => ({ openid }));
