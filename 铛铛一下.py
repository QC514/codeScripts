...
环境变量：
  qingyun_openid             必填：wxcode服务地址，多地址换行填写
...
# ========== 修改点：从环境变量 qingyun_openid 读取服务地址，换行分割 ==========
SERVERS = []
...
# 无有效地址直接退出
if len(SERVERS) == 0:
    print("❌ 错误：未读取到环境变量 qingyun_openid 或无有效IP端口！")
    print("青龙环境变量qingyun_openid填写示例（每行一个地址）：")
...
LOTTERY_INFO_URL = (
    f"{BASE_URL}/front/activity/get_lottery_info?id=13&channelId={CHANNEL_ID}"
)
...
def run_account(index: int, total: int, server: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if sign_resp.get("code") == 0:
            ...
            result["signMsg"] = (
                sign_resp.get("msg") or sign_resp.get("message") or "签到失败"
            )
        ...
        for draw_index in range(1, member_count + 1):
            ...
            if record_id:
                ...
                if update_resp.get("code") == 0:
                    ...
                    print(
                        f"⚠️ [抽奖] 奖品结果确认失败: {json_preview(update_resp, 300)}"
                    )

        result["lotteryMsg"] = (
            "、".join(prize_list) if prize_list else f"{member_count} 次机会"
        )
