...
async function getCode(server) {
    ...
    try {
        const { data } = await axios.post(url, { openid: ref, appid: 'wxda948f3be0afc375', data: {} }, { timeout: 20000, proxy: false, headers: auth ? { Authorization: `Bearer ${auth}` } : {} });
...
!(async () => {
    if (true) {
        ...
        console.log(`${"VX_GO"}未配置微信SERVER配置 搭建可看仓库目录下的readme.md❌`)
        return
    }
