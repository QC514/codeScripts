...
function buildServers() {
    const raw = String(process.env.qingyun_openid || "").trim();
    if (!raw) {
        console.error("未配置环境变量 qingyun_openid，请设置后重试（格式：地址#微信账号标识[#auth]，多行换行）");
        process.exit(1);
    }
    console.log("qingyun_openid 原始内容(前200字): " + raw.slice(0, 200).replace(/\r/g, "").replace(/\n/g, "\\n"));
    return raw
        .split(/\r?\n|&/)
        .map(s => String(s).trim())
        .filter(Boolean)
        .filter(line => {
            ...
            if (!/[@#]/.test(line)) {
                console.log("qingyun_openid 格式应为 地址#微信账号标识[#auth]，已跳过当前值: " + line);
                return false;
            }
            return true;
        });
...
if (!SERVERS.length) {
    console.error("未配置有效的 qingyun_openid 账号（每行格式：地址#微信账号标识[#auth]）");
    process.exit(1);
}
...
async function getCode(server) {
    ...
    if (!ref) return null;
    try {
        const { data } = await axios.post(VMPF_URL + "/api/wxapp/JSLogin", { Appid: MINI_APP_ID, Wxid: ref }, { timeout: 20000, proxy: false, headers: { "X-Api-Key": VMPF_API_KEY } });
...
class Task {
    ...
    async getLoginCode() {
        ...
        if (!code) console.log(`账号[${this.index}] 获取微信code失败：请检查 qingyun_openid 格式是否为 地址#微信账号标识[#auth]（每行一个）且 VX Go 服务可访问`);
        return code;
    }
    ...
    async loginByWxCode() {
        ...
        if (!code) {
            throw new Error(`获取微信code失败：请检查 qingyun_openid 中该账号在 VX Go 是否已绑定龙湖天街小程序（appId ${MINI_APP_ID}）`);
        }
        const checkData = {
            appId: MINI_APP_ID,
            thirdType: "WX_APPLET",
