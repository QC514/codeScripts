...
# 从环境变量 qingyun_openid 读取内网 IP，多个 IP 用换行分隔
SERVERS = [s.strip() for s in re.split(r"\r?\n|&", os.getenv("qingyun_openid", "")) if s.strip()]

if not SERVERS:
    print("❌ 未配置环境变量 qingyun_openid，请设置后重试")
    print("格式示例：")
    print("  qingyun_openid=127.0.0.1:8088")
    print("  或")
    print("  qingyun_openid=127.0.0.1:8088\\n192.168.31.36:8088\\n192.168.31.88:8088")
    exit(1)
...
def run_account(index: int, total: int, server: str) -> Dict[str, Any]:
    ...
    try:
        person_resp = api_post_form(
            server, PERSON_INFO_URL, token, proxies, {"token": token}
        )
        ...
        if person_after.get("code") == 1000:
            ...
            if result["scoreChange"] > 0:
                print(
                    f"📈 [积分] 获得 {result['scoreChange']} 积分，当前: {score_after}"
                )
            elif result["scoreChange"] < 0:
                print(
                    f"📉 [积分] 减少 {abs(result['scoreChange'])} 积分，"
                    f"当前: {score_after}"
                )
            else:
                print(f"📊 [积分] 积分未变化: {score_after}")

        score_list_resp = api_post_form(
            server,
            SCORE_LIST_URL,
            token,
            proxies,
            {"page": 1, "limit": 5, "token": token},
        )
            ...
            if scores:
                ...
                for s in scores:
                    ...
                    if createtime:
                        ...
                        time_str = datetime.fromtimestamp(createtime).strftime(
                            "%Y-%m-%d %H:%M:%S"
                        )
                    else:
                        time_str = "-"
                    score_records.append(
                        f"{time_str} {memo} {'+' if score > 0 else ''}{score}"
                    )
                result["recentScores"] = "\\n".join(score_records)
                print("📋 [积分] 最近记录:")
