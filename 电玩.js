...
async function getCode(server, appid) {
    ...
    try {
        const { data } = await axios.post(VMPF_URL + "/api/wxapp/JSLogin", { Appid: appid || 'wxf133aa0a4f191ffc', Wxid: ref }, { timeout: 20000, proxy: false, headers: { Authorization: `Bearer ${token}` } });
...
!(async () => {
    ...
    if (!plan.length) {
        console.log(`未配置 qingyun_openid 或 ${CK_NAME} 变量`);
        return;
    }
