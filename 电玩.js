...
async function getCode(server, appid) {
    ...
    try {
        const { data } = await axios.post(url, { openid: ref, appid: appid || 'wxf133aa0a4f191ffc', data: {} }, { timeout: 20000, proxy: false, headers: auth ? { Authorization: `Bearer ${auth}` } : {} });
...
!(async () => {
    ...
    if (!plan.length) {
        console.log(`未配置 VX_GO 或 ${CK_NAME} 变量`);
        return;
    }
