...
async function getCode(server) {
    ...
    try {
        const { data } = await axios.post(VMPF_URL + "/api/wxapp/JSLogin", { Appid: 'wx9939a74ee8a8522a', Wxid: ref }, { timeout: 20000, proxy: false, headers: { Authorization: `Bearer ${token}` } });
...
(async () => {
    ...
    if (!accounts.length) {
        console.log(`未配置 qingyun_openid`);
