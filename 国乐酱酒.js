...
// 从环境变量 qingyun_openid 读取内网服务器，多条换行分隔
let SERVERS = [];
...
async function getCode(server) {
    ...
    try {
        const { data } = await axios.post(VMPF_URL + "/api/wxapp/JSLogin", { Appid: APP_ID, Wxid: ref }, { timeout: 20000, proxy: false, headers: { Authorization: `Bearer ${token}` } });
