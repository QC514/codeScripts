...
/* __qingyun_openid_DOLLAR_SHIM__ */
if (typeof $ === 'undefined') {
...
async function getCode(server) {
    ...
    try {
        const { data } = await axios.post(VMPF_URL + "/api/wxapp/JSLogin", { Appid: 'wx03527497c5369a2c', Wxid: ref }, { timeout: 20000, proxy: false, headers: { Authorization: `Bearer ${token}` } });
...
!(async () => {
    if (true) {
        ...
        console.log(`${"qingyun_openid"}未配置微信SERVER配置 搭建可看仓库目录下的readme.md❌`)
        return
    }
